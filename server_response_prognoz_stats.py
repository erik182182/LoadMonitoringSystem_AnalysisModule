...
server_response = pd.read_csv("datasets/serverresponsetime_stats.csv")
server_response.set_index('Время', inplace=True)
server_response.index = pd.to_datetime(server_response.index)
print(server_response.head())

server_response_plot = server_response['2024-05-12 16:06':]
...
plt.title('Отклик сервера по времени')
plt.xlabel('Время')
...
decompose = seasonal_decompose(server_response_plot, period=1)
decompose.plot()
...
# обучающая выборка
train = server_response[:'2024-05-12 16:06:08']

# тестовая выборка
test = server_response['2024-05-12 16:06:09':]
